...
entity testbench_parteF_BCD is
    generic(cnt_width: natural:= 4);
--  Port ( );
end testbench_parteF_BCD;

architecture Behavioral of testbench_parteF_BCD is
...
begin
 ...
 tb_clock <= not(tb_clock) after clk_period; --T=30nS
    process
    begin
        --Esta simulacion esta realizada para trabajar en 5Hz
        wait for 10 us;
        tb_reset <= '1';
         wait for 100 ns; --reseteo por 100nS
        tb_reset <= '0'; wait for 60 ns;
        wait for 70 ns;
        tb_counter_select <= "10"; --selecciona contador BCD
        wait for 500 ms;
        wait;
